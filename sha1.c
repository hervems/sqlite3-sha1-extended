...
** X is NULL. If you use raw mode, it returns SHA1 in raw, no in hexa values.
**
...
static void hash_step(SHA1Context * p,	/* Add content to this context */
...
static void hash_step_vformat(SHA1Context * p,	/* Add content to this context */
			      const char *zFormat, ...)
{
	va_list ap;
	int n;
...
static void hash_finish(SHA1Context * p,	/* The SHA1 context to finish and render */
			char *zOut,	/* Store hexadecimal hash here */
			bool rawMode	/* Raw mode */
    )
{
	unsigned int i;
	...
	for (i = 0; i < 20; i++) {
		digest[i] =
		    (unsigned char)((p->state[i >> 2] >> ((3 - (i & 3)) * 8)) &
				    255);
	}
...
static void sha1Func(sqlite3_context * context, int argc, sqlite3_value ** argv)
{
	...
	if (argc == 1) {
		hash_finish(&cx, zOut, false);
		sqlite3_result_text(context, zOut, 40, SQLITE_TRANSIENT);
		return;
	}

	hash_finish(&cx, zOut, true);
...
*/
static void
sha1QueryFunc(sqlite3_context * context, int argc, sqlite3_value ** argv)
{
	...
	while (zSql[0]) {
		...
		if (!sqlite3_stmt_readonly(pStmt)) {
			char *zMsg = sqlite3_mprintf("non-query: [%s]",
						     sqlite3_sql(pStmt));
			sqlite3_finalize(pStmt);
			sqlite3_result_error(context, zMsg, -1);
			sqlite3_free(zMsg);
		     ...
		     char **pzErrMsg, const sqlite3_api_routines * pApi)
{
	...
	if (rc == SQLITE_OK) {
		rc = sqlite3_create_function(db, "sha1", 2, SQLITE_UTF8, 0,
					     sha1Func, 0, 0);
	}

	if (rc == SQLITE_OK) {
		rc = sqlite3_create_function(db, "sha1_query", 1, SQLITE_UTF8,
					     0, sha1QueryFunc, 0, 0);
	}
	return rc;
}
